...
for date in date_ls[:]:
    ...
    data = np.fromfile(save_path, dtype=np.float32).reshape(-1, 12)  
    data_ls.append(data)

# 合并数据并计算收益率
df = np.concatenate(data_ls, axis=0)
cum_vol = df[:,5].cumsum()
cum_vol = (cum_vol - (cum_vol//vol_num *vol_num))[:-1]
next_vol = df[:,5][1:]
index = (cum_vol+next_vol)>=vol_num
bar = df[1:,:][index]
close = bar[:,4]
r = np.diff(close) / close[:-1]
bar.tofile(os.path.join(r'C:\Users\yy\Desktop\HFT_zxy\bar_data', symbol+'.dat'))
print(r.shape)
...
sns.kdeplot(r_sampled, label="Standardized Returns (KDE)", color="blue", linestyle="--")
