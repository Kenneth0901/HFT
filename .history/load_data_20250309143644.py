...
symbol = ['BTCUSDT']
for date in date_ls[0]:
    timestamp = int(date.timestamp() * 1000)
    date_str = date.strftime("%Y%m%d")
    data_ls = []
    for i in range(108):
        start_time = timestamp + i*limit*1000
        url = os.path.join(BASE_URL, kline, '?symbol=', symbol, '&interval=1s&limit=', limit, '&startTime=', start_time, '&endTime=', start_time+(limit*1000))
        ...
        if status_code == 200:
            ...
            if data:
                ...
                logger.warning(f'No data returned for {symbol} at {start_time}')
    df = np.concatenate(data_ls,axis=0)
    np.tofile(os.path.join(r'C:\Users\yy\Desktop\HFT_zxy\data', date_str, symbol,'1sbar.dat'))
    logger.info(f'Finished downloading data for {date_str}')
