...
class StableTrendStrategy(Strategy):
    ...
    def __init__(self, bars, port: NaivePortfolio, events):
        ...
        # 策略参数
        self.entry_period = 30      # 通道周期
        self.atr_period = 20        # 波动率周期
        self.risk_ratio = 0.01      # 单笔风险比例
        self.trend_filter = 100     # 趋势过滤均线
    ...
    def calculate_signals(self, event):
        if event.type == 'MARKET':
            ...
            for s in self.symbol_list:
                ...
                # 修正后的信号生成
                if price >= data['upper']:
                    self.events.put(SignalEvent(s, timestamp, 'LONG', self.initial_capital/bars[-1][5]*0.8))  # 使用正确的timestamp
                    
                # elif price <= data['lower']:
                #     self.events.put(SignalEvent(s, timestamp, 'SHORT', self.initial_capital/bars[-1][5]*0.1))  # 使用正确的timestamp

                for order in [o for o in self.port.order_history if o['symbol'] == s]:
                        if  bars[-1][5]>order['price']*1.05:
                            order_event = OrderEvent(order['symbol'], 'MKT', order['quantity'], 'SELL', order_mark='CLOSE')
                            self.events.put(order_event)
                        else: new_order_history.append(order)
            self.port.order_history = new_order_history