        ...
        if event.type == 'MARKET':
            ...
            for s in self.symbol_list:
                ...
                if price >= data['upper'] and price >=data['ma_filter']:
                    self.events.put(SignalEvent(s, timestamp, 'LONG', self.initial_capital*self.risk_ratio / self.N*data['atr']))  # 使用正确的timestamp
                    pass
                    
                elif price <= data['lower'] and price <= data['ma_filter']:
                    self.events.put(SignalEvent(s, timestamp, 'SHORT', self.initial_capital*self.risk_ratio / self.N*data['atr']))  # 使用正确的timestamp
                    pass

                for order in [o for o in self.port.order_history if o['symbol'] == s]:
                        if order['direction'] =='BUY':
                            if  bars[-1][5]>order['price']+3*data['atr'] or bars[-1][5]<order['price']-3*data['atr'] or order['flag']==2000:
