...
class HistoricDataHandler(DataHandler):
    ...
    def _open_convert_files(self):
        ...
        for s in self.symbol_list:
            ...
            self.symbol_data[s] = pd.DataFrame(np.fromfile(os.path.join(self.dir, s+'.dat'),dtype=np.float32).reshape(-1,12))[:50000]
            ...
            self.symbol_data[s].iloc[:,4].plot()

            self.symbol_data[s]['start_time'] = pd.to_datetime(self.symbol_data[s]['start_time'], unit='ms')
            self.symbol_data[s]['end_time'] = pd.to_datetime(self.symbol_data[s]['end_time']/1000, unit='s')
            self.symbol_data[s].set_index('end_time', inplace=True)
