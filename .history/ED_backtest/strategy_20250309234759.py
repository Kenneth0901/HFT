...
class StableTrendStrategy(Strategy):
    ...
    def __init__(self, bars, port: NaivePortfolio, events):
        ...
        # 策略参数
        self.entry_period = 64      # 通道周期
        self.atr_period = 32        # 波动率周期
        self.risk_ratio = 1      # 单笔风险比例
        self.trend_filter = 300     # 趋势过滤均线
        self.N = 10
            ...
            for s in self.symbol_list:
                ...
                for order in [o for o in self.port.order_history if o['symbol'] == s]:
                        if order['direction'] =='BUY':
                            if  bars[-1][5]>order['price']+2*self.N*data['atr'] or bars[-1][5]<order['price']-self.N*data['atr'] or order['flag']==100:
                            ...
                            if  bars[-1][5]<order['price']-2*self.N*data['atr'] or bars[-1][5]>order['price']+self.N*data['atr'] or order['flag']==200:
                                self.events.put(SignalEvent(s, timestamp, 'EXIT',0))
