    ...
    def calculate_signals(self, event):
        if event.type == 'MARKET':
            ...
            for s in self.symbol_list:
                ...
                # 修正后的信号生成
                if price >= data['upper']:
                    # self.events.put(SignalEvent(s, timestamp, 'LONG', self.initial_capital/bars[-1][5]*0.9))  # 使用正确的timestamp
                    pass
                    
                elif price <= data['lower']:
                    self.events.put(SignalEvent(s, timestamp, 'SHORT', self.initial_capital/bars[-1][5]*0.9))  # 使用正确的timestamp

                for order in [o for o in self.port.order_history if o['symbol'] == s]:
                        if order['direction'] =='BUY':
                            if  bars[-1][5]>order['price']*1.3 or bars[-1][5]<order['price']*0.95 or order['flag']==20:
                                self.events.put(SignalEvent(s, timestamp, 'SHORT', self.initial_capital/bars[-1][5]*0.9))
                                self.events.put(order_event)
                                order['flag'] += 1
                            else: new_order_history.append(order)
                        else:
                            if  bars[-1][5]<order['price']*0.8 or bars[-1][5]>order['price']*1.05 or order['flag']==20:
                                order_event = OrderEvent(order['symbol'], 'MKT', order['quantity'], 'BUY', order_mark='CLOSE')
                                self.events.put(order_event)
                                order['flag'] += 1
                            else: new_order_history.append(order)
