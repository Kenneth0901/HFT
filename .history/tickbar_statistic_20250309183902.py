...
for date in date_ls[:]:
    ...
    data = np.fromfile(save_path, dtype=np.float32).reshape(-1, 12)[:, [4,5]]  # 提取第 4 列（假设是价格）
    data_ls.append(data)

# 合并数据并计算收益率
df = np.concatenate(data_ls)
cum_vol = df[:,1].cumsum()
cum_vol = (cum_vol - (cum_vol//1000 *1000))[:-1]
next_vol = df[:,1][1:]
index = (cum_vol+next_vol)>=1000
df = df[1:,:][index]
r = np.diff(df[:,0]) / df[:-1,0]
